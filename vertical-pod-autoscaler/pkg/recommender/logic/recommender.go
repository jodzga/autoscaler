...
var (
	...
	podMinRssMb = flag.Float64("pod-recommendation-min-rss-mb", 1, `Minimum RSS recommendation for a pod`)
...
func (r *podResourceRecommender) GetRecommendedPodResources(containerNameToAggregateStateMap model.ContainerNameToAggregateStateMap) RecommendedPodResources {
	...
	for containerName, aggregatedContainerState := range containerNameToAggregateStateMap {
		klog.V(4).Infof("Estimating resources for container %s, %+v", containerName, aggregatedContainerState)
		recommendation[containerName] = recommender.estimateContainerResources(aggregatedContainerState)
		klog.V(4).Infof("Recommendation for container %s: %+v", containerName, recommendation[containerName])
	}
	return recommendation
}
