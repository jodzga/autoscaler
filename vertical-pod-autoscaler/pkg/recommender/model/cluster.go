...
func (cluster *ClusterState) RecordOOM(containerID ContainerID, timestamp time.Time, memoryType ResourceName, limitMemory ResourceAmount) error {
	pod, podExists := cluster.Pods[containerID.PodID]
	if !podExists {
		return NewKeyError(containerID.PodID)
	}
	...
	err := containerState.RecordOOM(timestamp, memoryType, limitMemory)
	if err != nil {
